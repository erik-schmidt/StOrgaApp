...
public class TimeTableObjectService extends CheckMatrNrClass {
    ...
    public ResponseEntity getAllTimeTableObjectsByFieldOfStudyAndSemester(String matrNr){
        try{
            ...
            List<TimeTableObject> timeTableObjectList = timeTableObjectRepository.findAllByFieldOfStudyAndSemester(fieldOfStudyAndSemester);
    ...
    public ResponseEntity getAllTimeTableObjectsByStartTime(LocalDateTime date){
        try{
            List<TimeTableObject> timeTableObjectList = timeTableObjectRepository.findAllByStartTimeDate(date);
            return new ResponseEntity(timeTableObjectList, HttpStatus.OK);
        }catch (Exception e){
            logger.error(e.getClass() + " " + e.getMessage());
            return new ResponseEntity(e.getMessage(), HttpStatus.BAD_REQUEST);
